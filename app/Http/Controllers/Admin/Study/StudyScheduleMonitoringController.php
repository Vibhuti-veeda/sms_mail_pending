...
class StudyScheduleMonitoringController extends GlobalController
{
    ...
    public function studyActivityMonitoringList(Request $request){
      ...
      /* echo "<pre>";
        ...
        if(isset($request->ref) && $request->ref != ''){
            $filter = 1;
            $status = $request->ref;
            if (Auth::guard('admin')->user()->role_id == 1 || Auth::guard('admin')->user()->role_id == 2 || Auth::guard('admin')->user()->role_id == 3 || Auth::guard('admin')->user()->role_id == 4 || Auth::guard('admin')->user()->role_id == 5 || Auth::guard('admin')->user()->role_id == 6 || Auth::guard('admin')->user()->role_id == 10 || Auth::guard('admin')->user()->role_id == 14 || Auth::guard('admin')->user()->role_id == 16 || Auth::guard('admin')->user()->role_id == 17 || Auth::guard('admin')->user()->role_id == 18 || Auth::guard('admin')->user()->role_id == 19 || Auth::guard('admin')->user()->role_id == 20 || Auth::guard('admin')->user()->role_id == 21) {
                $query->whereIn('activity_type',[113,114,115,116])->whereHas('studyNo', function($q) use($status){ $q->where('activity_status',$status);});
            } elseif(Auth::guard('admin')->user()->role_id == 11 || Auth::guard('admin')->user()->role_id == 12){
        ...
        if($request->refStatus == 'ONGOING'){
            $activityStatusName = 'ONGOING';
        } else if($request->refStatus == 'COMPLETED'){
            $activityStatusName = 'COMPLETED';
        } else if($request->refStatus == 'DELAY'){
            $activityStatusName = 'DELAY';
        } else if($request->refStatus == 'UPCOMING'){
            $activityStatusName = 'UPCOMING';
        }

        if($request->refStatus != '') {
            $activityStatus = $query->select('id', 'study_id', 'activity_id', 'activity_status', 'activity_name', 'group_no', 'period_no', 
                                    ...
                                    ->with([
                                        ...
                                        'studyNo' => function($q) { 
                                            $q->select('id', 'project_manager', 'sponsor', 'cr_location', 'br_location', 'study_no', 'study_type')
                                              ->where('is_active', 1)
                                              ->where('is_delete', 0)
                                              ->with(
                                                [
                                    ...
                                    ->where('activity_status',$request->refStatus)
                                    ->orderBy('scheduled_start_date', 'ASC')
                                    ->get();

        } else if($filter == 1 || $request->ref != ''){
            $activityStatus = $query->select('id', 'study_id', 'activity_id', 'activity_status', 'activity_name', 'group_no', 'period_no', 
                                    'scheduled_start_date', 'actual_start_date', 'scheduled_end_date', 'actual_end_date', 'start_delay_remark', 'end_delay_remark', 'start_delay_reason_id', 'end_delay_reason_id', 'actual_start_date_time', 'actual_end_date_time', 'activity_version_type', 'activity_version', 'activity_type')
                                    ->with([
                                        'activityName' => function($q) {
                                            $q->select('id', 'activity_status', 'activity_status_code');
                                        },
        ...
        if(Auth::guard('admin')->user()->role_id == 5 || Auth::guard('admin')->user()->role_id == 7 || Auth::guard('admin')->user()->role_id == 8 || Auth::guard('admin')->user()->role_id == 10 || Auth::guard('admin')->user()->role_id == 11 || Auth::guard('admin')->user()->role_id == 12 || Auth::guard('admin')->user()->role_id == 13){
            ...
            //added studySubTypes studySubType
        return view('admin.study.study_schedule_monitoring.study_activity_monitoring_list', compact('activityStatus', 'ref', 'filter', 'status', 'studies', 'studyName', 'activities', 'activityName', 'startDate', 'endDate', 'scheduleStartDate', 'scheduleEndDate', 'sponsors', 'crLocation', 'crLocationName', 'brLocation', 'brLocationName', 'activityStatusName', 'sponsorName', 'actualStartDate', 'actualEndDate', 'actualEndStartDate', 'actualEndEndDate', 'projectManagers', 'projectManagerName', 'projectManagerActivities','activityStatusMaster', 'studySubTypes', 'studySubType'));
    }
    ...
    public function saveStudyScheduleActualStartDateModal(Request $request) {
        ...
        $saveStudyScheduleActualStartDate = StudySchedule::where('id', $request->id)->where('is_active', 1)->where('is_delete', 0)->first();
        
        if(!is_null($saveStudyScheduleActualStartDate)) {
            if((isset($request->actual_start_date)) && ($request->actual_start_date != '')) {
                $a = date('d-m-Y', strtotime($request->actual_start_date));     
                $saveStudyScheduleActualStartDate->actual_start_date = $this->convertDt($a);
            }

            if($saveStudyScheduleActualStartDate->actual_start_date <= $saveStudyScheduleActualStartDate->scheduled_start_date){
                $saveStudyScheduleActualStartDate->start_delay_reason_id = null;
                $saveStudyScheduleActualStartDate->start_delay_remark = null;
            } else if($saveStudyScheduleActualStartDate->actual_start_date > $saveStudyScheduleActualStartDate->scheduled_start_date){
                if(($request->start_delay_reason_id != '0')) {
                    $saveStudyScheduleActualStartDate->start_delay_reason_id = $request->start_delay_reason_id;
                    $saveStudyScheduleActualStartDate->start_delay_remark = null;
                } else if($request->start_delay_reason_id == '0'){
                    $saveStudyScheduleActualStartDate->start_delay_reason_id = $request->start_delay_reason_id;
                    $saveStudyScheduleActualStartDate->start_delay_remark = $request->start_delay_remark;
                } 
            }
            
            if (Auth::guard('admin')->user()->id != '') {
                $saveStudyScheduleActualStartDate->updated_by_user_id = Auth::guard('admin')->user()->id;
            }
             
            $saveStudyScheduleActualStartDate->save();
        }

        $saveStudyScheduleActualStartDateTrail = StudyScheduleTrail::where('study_schedule_id', $request->id)
                                                                   ->where('is_active', 1)
                                                                   ->where('is_delete', 0)
                                                                   ->orderBy('id', 'DESC')
                                                                   ->first();

        if(!is_null($saveStudyScheduleActualStartDateTrail)) {
            if((isset($request->actual_start_date)) && ($request->actual_start_date != '')) {
                $a = date('d-m-Y', strtotime($request->actual_start_date));     
                $saveStudyScheduleActualStartDateTrail->actual_start_date = $this->convertDt($a);
            }

            if($saveStudyScheduleActualStartDateTrail->actual_start_date <= $saveStudyScheduleActualStartDateTrail->scheduled_start_date){
                $saveStudyScheduleActualStartDateTrail->start_delay_reason_id = null;
                $saveStudyScheduleActualStartDateTrail->start_delay_remark = null;
            } else if($saveStudyScheduleActualStartDateTrail->actual_start_date > $saveStudyScheduleActualStartDateTrail->scheduled_start_date){
                if(($request->start_delay_reason_id != '0')) {
                    $saveStudyScheduleActualStartDateTrail->start_delay_reason_id = $request->start_delay_reason_id;
                    $saveStudyScheduleActualStartDateTrail->start_delay_remark = null;
                } else if($request->start_delay_reason_id == '0') {
                    $saveStudyScheduleActualStartDateTrail->start_delay_reason_id = $request->start_delay_reason_id;
                    $saveStudyScheduleActualStartDateTrail->start_delay_remark = $request->start_delay_remark;
                } 
            }

            if (Auth::guard('admin')->user()->id != '') {
                $saveStudyScheduleActualStartDateTrail->updated_by_user_id = Auth::guard('admin')->user()->id;
            }
         
            $saveStudyScheduleActualStartDateTrail->save();
        }
    ...
    public function saveStudyScheduleActualEndDateModal(Request $request) { 
        ...
        $saveStudyScheduleActualEndDate = StudySchedule::where('id', $request->id)->where('is_active', 1)->where('is_delete', 0)->first();

        if(!is_null($saveStudyScheduleActualEndDate)) {
            if((isset($request->actual_end_date)) && ($request->actual_end_date != '')) {
                $a = date('d-m-Y', strtotime($request->actual_end_date));     
                $saveStudyScheduleActualEndDate->actual_end_date = $this->convertDt($a);
            }

            if($saveStudyScheduleActualEndDate->actual_end_date <= $saveStudyScheduleActualEndDate->scheduled_end_date){
                $saveStudyScheduleActualEndDate->end_delay_reason_id = null;
                $saveStudyScheduleActualEndDate->end_delay_remark = null;
            } else if($saveStudyScheduleActualEndDate->actual_end_date > $saveStudyScheduleActualEndDate->scheduled_end_date){
                if(($request->end_delay_reason_id != '0')) {
                    $saveStudyScheduleActualEndDate->end_delay_reason_id = $request->end_delay_reason_id;
                    $saveStudyScheduleActualEndDate->end_delay_remark = null;
                } else if($request->end_delay_reason_id == '0'){
                    $saveStudyScheduleActualEndDate->end_delay_reason_id = $request->end_delay_reason_id;
                    $saveStudyScheduleActualEndDate->end_delay_remark = $request->end_delay_remark;
                } 
            }
            
            if (Auth::guard('admin')->user()->id != '') {
                $saveStudyScheduleActualEndDate->updated_by_user_id = Auth::guard('admin')->user()->id;
            }
             
            $saveStudyScheduleActualEndDate->save();
        }

        $saveStudyScheduleActualEndDateTrail = StudyScheduleTrail::where('study_schedule_id', $request->id)
                                                                 ->where('is_active', 1)
                                                                 ->where('is_delete', 0)
                                                                 ->orderBy('id', 'DESC')
                                                                 ->first();

        if(!is_null($saveStudyScheduleActualEndDateTrail)) {
            if((isset($request->actual_end_date)) && ($request->actual_end_date != '')) {
                $a = date('d-m-Y', strtotime($request->actual_end_date));     
                $saveStudyScheduleActualEndDateTrail->actual_end_date = $this->convertDt($a);
            }

            if($saveStudyScheduleActualEndDateTrail->actual_end_date <= $saveStudyScheduleActualEndDateTrail->scheduled_end_date){
                $saveStudyScheduleActualEndDateTrail->end_delay_reason_id = null;
                $saveStudyScheduleActualEndDateTrail->end_delay_remark = null;
            } else if($saveStudyScheduleActualEndDateTrail->actual_end_date > $saveStudyScheduleActualEndDateTrail->scheduled_end_date){
                if(($request->end_delay_reason_id != '0')) {
                    $saveStudyScheduleActualEndDateTrail->end_delay_reason_id = $request->end_delay_reason_id;
                    $saveStudyScheduleActualEndDateTrail->end_delay_remark = null;
                } else if($request->end_delay_reason_id == '0') {
                    $saveStudyScheduleActualEndDateTrail->end_delay_reason_id = $request->end_delay_reason_id;
                    $saveStudyScheduleActualEndDateTrail->end_delay_remark = $request->end_delay_remark;
                } 
            }

            if (Auth::guard('admin')->user()->id != '') {
                $saveStudyScheduleActualEndDateTrail->updated_by_user_id = Auth::guard('admin')->user()->id;
            }
         
            $saveStudyScheduleActualEndDateTrail->save();
        }
    ...
    // departmentScheduleTrackingList
    public function departmentScheduleTrackingList(Request $request){
        ...
        $query = Study::select('id','study_no', 'sponsor', 'study_status', 'study_result','global_priority_no')->where('is_active', 1)->where('is_delete', 0);
        ...
        if(Auth::guard('admin')->user()->role_id == '1'){
            $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
                                                },
                                                'drugDetails' => function($q) {
        ...
        } else if(Auth::guard('admin')->user()->role_id == '17'){
            if(($request->ref == 'ONGOING') || ($request->ref == 'COMPLETED') || ($request->ref == 'UPCOMING')){    
                $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
        ...
        } else if(Auth::guard('admin')->user()->role_id == '18'){
            if(($request->ref == 'ONGOING') || ($request->ref == 'COMPLETED') || ($request->ref == 'UPCOMING')){    
                $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
        ...
        } else if(Auth::guard('admin')->user()->role_id == '19'){
            if(($request->ref == 'ONGOING') || ($request->ref == 'COMPLETED') || ($request->ref == 'UPCOMING')){    
                $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
                                             ...
                                             ->whereHas('schedule', function($q) {
                                                $q->whereNotNull('scheduled_start_date')
                                                ->whereIn('responsibility_id', [12, 13])
                                                ->where('is_active', 1)
                                                ->where('is_delete', 0);
                                              })
                                             ->orderBy('global_priority_no', 'ASC')
        ...
        } else if(Auth::guard('admin')->user()->role_id == '20'){
            if(($request->ref == 'ONGOING') || ($request->ref == 'COMPLETED') || ($request->ref == 'UPCOMING')){    
                $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
                                             ...
                                             ->whereHas('schedule', function($q) {
                                                $q->whereNotNull('scheduled_start_date')
                                                ->whereIn('responsibility_id', [15, 9])
                                                ->where('is_active', 1)
                                                ->where('is_delete', 0);
                                              })
                                             ->orderBy('global_priority_no', 'ASC')
        ...
        } else if(Auth::guard('admin')->user()->role_id == '21'){
            if(($request->ref == 'ONGOING') || ($request->ref == 'COMPLETED') || ($request->ref == 'UPCOMING')){    
                $depScheduleTrackingList =  $query->with([
                                                'sponsorName' => function($q) {
                                                    $q->select('id','sponsor_name');
                                             ...
                                             ->where('cr_location',Auth::guard('admin')->user()->location_id)
                                             ->orderBy('global_priority_no', 'ASC')
                                             ->get();
        ...
        return view('admin.study.study_schedule_monitoring.department_schedule_tracking_list', compact('depScheduleTrackingList','studyStatus', 'filter', 'studyStatusName','ref'));
    }

    // departmentActivityTrackingList
    public function departmentActivityTrackingList(Request $request){
        ...
        $studiesQuery = Study::select('id','study_no')->where('is_active', 1)->where('is_delete', 0);
        $activitiesQuery = ActivityMaster::select('id','activity_name')->where('is_active', 1)->where('is_delete', 0);
        $sponsors = SponsorMaster::where('is_active', 1)->where('is_delete', 0)->get();
        $crLocation = LocationMaster::where('location_type', 'CRSITE')->where('is_active', 1)->where('is_delete', 0)->get();
        $brLocation = LocationMaster::where('location_type', 'BRSITE')->where('is_active', 1)->where('is_delete', 0)->get();
        $projectManagers = Admin::whereIn('role_id', ['2', '3'])->where('is_active', 1)->where('is_delete', 0)->get();
        ...
        $query = StudySchedule::select('id', 'study_id', 'activity_id', 'activity_status', 'activity_name', 'group_no', 'period_no', 
                                    'scheduled_start_date', 'actual_start_date', 'scheduled_end_date', 'actual_end_date', 'start_delay_remark', 'end_delay_remark', 'start_delay_reason_id', 'end_delay_reason_id', 'actual_start_date_time', 'actual_end_date_time', 'activity_version_type', 'activity_version')
                              ->where('is_active', 1)
                              ->where('is_delete', 0)
        ...
        if(isset($request->ref) && $request->ref != ''){
            $filter = 1;
            $status = $request->ref;
            if ((Auth::guard('admin')->user()->role_id == 1) || (Auth::guard('admin')->user()->role_id == 16) || (Auth::guard('admin')->user()->role_id == 17) || (Auth::guard('admin')->user()->role_id == 18) || (Auth::guard('admin')->user()->role_id == 19) || (Auth::guard('admin')->user()->role_id == 20)) {
                $query->whereHas('studyNo', function($q) use($status){ $q->where('activity_status',$status);});
            } elseif(Auth::guard('admin')->user()->role_id == 21){
                $query->whereHas('studyNo', function($q) use($status){ $q->where('activity_status',$status)->where('cr_location', Auth::guard('admin')->user()->location_id);});    
            }
        }
        ...
        if(Auth::guard('admin')->user()->role_id == '1'){
            $studies = $studiesQuery->whereHas('schedule', function($q) {
                                            $q->whereNotNull('scheduled_start_date')
                                            ->where('is_active', 1)
                                            ->where('is_delete', 0);
                                        })
            ...
            $activities = $activitiesQuery->where('responsibility', 11)->get();

            $depActivityTrackingList = $query->where('responsibility_id', 11)
                                               ->with([
            ...
            if($request->refStatus != ''){
                ...
                $depActivityTrackingList = $query->where('responsibility_id', 11)
                                                ->where('scheduled_start_date', $startDate)
            ...
            if($request->refStatus != ''){
                ...
                $activities = $activitiesQuery->where('responsibility', 7)->get();

                $depActivityTrackingList = $query->where('responsibility_id', 7)
                                                ->with([
                                                    'activityMaster' => function($q){
                                                        $q->select('id','activity_name')
                                                          ->where('is_active', 1)
            ...
            if($request->refStatus != ''){
                ...
                $activities = $activitiesQuery->where('responsibility', 8)->get();

                $depActivityTrackingList = $query->where('responsibility_id', 8)
                                                ->with([
                                                    'activityMaster' => function($q){
                                                        $q->select('id','activity_name')
                                                          ->where('is_active', 1)
            ...
            if($request->refStatus != ''){
                ...
                $activities = $activitiesQuery->whereIn('responsibility', [12, 13])->get();

                $depActivityTrackingList = $query->whereIn('responsibility_id', [12, 13])
                                                ->with([
                                                    'activityMaster' => function($q){
                                                        $q->select('id','activity_name')
                                                          ->where('is_active', 1)
            ...
            if($request->refStatus != ''){
                $studies = $studiesQuery->whereHas('schedule', function($q) {
                                                $q->whereNotNull('scheduled_start_date')
                                                ->whereIn('responsibility_id', [9, 15])
                                                ->where('is_active', 1)
                                                ->where('is_delete', 0);
                                            })
                                        ->get();

                $activities = $activitiesQuery->whereIn('responsibility', [9, 15])->get();

                $depActivityTrackingList = $query->whereIn('responsibility_id', [9, 15])
                                                ->with([
                                                    'activityMaster' => function($q){
                                                        $q->select('id','activity_name')
                                                          ->where('is_active', 1)
            ...
            if($request->refStatus != ''){
                ...
                $depActivityTrackingList = $query->where('responsibility_id', 11)
                                                ...
                                                ->whereHas('studyNo', function($q){
                                                    $q->where('cr_location', Auth::guard('admin')->user()->location_id);
                                                })
                                                ->orderBy('scheduled_start_date', 'ASC')
                                                ->where('activity_status',$request->refStatus)
                                                ->get();
        ...
        return view('admin.study.study_schedule_monitoring.department_activity_tracking_list', compact('filter','depActivityTrackingList', 'projectManagers', 'studies', 'activities', 'sponsors', 'activityStatusMaster','crLocation', 'brLocation', 'startDate', 'endDate', 'scheduleStartDate', 'scheduleEndDate', 'actualStartDate', 'actualEndDate', 'actualEndStartDate', 'actualEndEndDate', 'studySubTypes', 'projectManagerName', 'studyName', 'activityName', 'sponsorName', 'activityStatusName', 'status', 'crLocationName', 'brLocationName', 'studySubType'));     
    }
