...
class StudyScheduleController extends GlobalController
{
    ...
    **/
    ...
    // get Delay records
    public function getDelayActivityRecords(){
        
        $responsibilityIds = [7, 8, 9, 11, 12, 13, 15];
        $delayActivities = StudySchedule::select('id', 'study_id', 'activity_id', 'activity_name', 'activity_status', 'activity_type', 'scheduled_start_date', 'responsibility_id')
        ...
        if(!is_null($responsibilityIds)){
            foreach ($responsibilityIds as $rbik => $rbiv){
                ($delayActivity[$rbiv] = $delayActivities->where('responsibility_id', $rbiv));
            }
        }                                
        return view('admin.mail.delay_activities_list',compact('delayActivity','responsibilityIds'));
    }
