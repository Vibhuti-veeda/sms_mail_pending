...
class AdminController extends GlobalController
{
    ...
    public function index(){
        ...
        $activities = ActivityMaster::where('responsibility', Auth::guard('admin')->user()->role_id)->where('is_delete', 0)->where('is_active', 1)->get('id')->toArray();
        
        $crlocation = Study::where('cr_location',Auth::guard('admin')->user()->location_id)
                            ->where('is_active', 1)
                            ->where('is_delete', 0)
                            ->get('id')
                            ->toArray();

        $brlocation = Study::where('br_location',Auth::guard('admin')->user()->location_id)->where('is_active', 1)->where('is_delete', 0)->get('id')->toArray();
    ...
    public function changeDashboardView(Request $request){
        ...
        return view('admin.dashboard.dashboard',compact('totalCompletedStudy', 'totalOngoingStudy', 'totalUpcomingStudy', 'totalPreCompleted' ,'totalCompleted', 'totalUpcoming', 'totalPreUpcoming', 'totalOngoing', 'totalPreOngoing', 'totalDelay' , 'totalPreDelay', 'pmCount', 'graphName', 'graphDelay'));
            ...
            $html = view('admin.dashboard.hod_dashboard',compact('totalCompletedStudy', 'totalOngoingStudy', 'totalUpcomingStudy', 'totalCompleted', 'totalUpcoming', 'totalOngoing', 'totalDelay'))->render();
            ...
            $totalCompletedStudy = Study::where('cr_location',Auth::guard('admin')->user()->location_id)
                                        ->where('is_active', 1)
                                        ->where('is_delete', 0)
                                        ->where('study_status', 'COMPLETED')
                                        ->whereHas('schedule', function($q) {
            ...
            $totalOngoingStudy = Study::where('cr_location',Auth::guard('admin')->user()->location_id)
                                      ->where('is_active', 1)
                                      ->where('is_delete', 0)
                                      ->where('study_status', 'ONGOING')
                                      ->whereHas('schedule', function($q) { 
            ...
            $totalUpcomingStudy = Study::where('cr_location',Auth::guard('admin')->user()->location_id)
                                       ->where('is_active', 1)
                                       ->where('is_delete', 0)
                                       ->where('study_status', 'UPCOMING')
                                       ->whereHas('schedule', function($q) {
